...
int main(int iargc, char **argv){
    ...
    for( int i = 1; i < iargc; i++ ){
        ...
        if(par=="--new_tpl" ){
            new_tpl=true;
            std::cout<<"You are using the new ft2 template \n";
        }
    }
    ...
    for( int i = 1; i < iargc; i++ ){
        ...
        if(par=="-FT2_fits_File" ) {
            FITS(iargc, argv, old_tpl);
        }
        else if(par=="-FT2_txt_File" )   {
            TXT(iargc, argv, old_tpl);
        }
    }
...
void FITS(int iargc, char **argv, bool old_tpl){
    try{
        ...
        fitsGen::Ft2File ft2_merged  (ft2_name_merged, 1, "SC_DATA", "/afs/slac.stanford.edu/u/gl/tramacer/fitsGen/ScienceTools-LATEST-1-2481/data/ft2.tpl");
