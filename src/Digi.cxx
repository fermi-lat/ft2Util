...
void FT2::Digi_FT2(FT2 &FT2) {
    ...
    //-----------------------------------------------------------
    //Find Tstart and Tstop of the Digi File
    //-----------------------------------------------------------
    ...
    //command line Tsart and Tstop provided
    if (FT2.DigiCut == true) {
        printf("---------------------------------------------------\n");
        printf("Digi times forced from command line\n");
        printf("---------------------------------------------------\n");
        T->GetEntry(Digi_i);
        DigiTime = evt->getTimeStamp();
        std::cout << setprecision(20) << "Tstart  of the DigiFile  = " << DigiTime << "\n";

        do {
            T->GetEntry(Digi_i);
            DigiTime = evt->getTimeStamp();
            Digi_i++;
        } while (DigiTime < FT2.DigiCutTstart && Digi_i < Digi_nEvt);
        ...
        std::cout << "RunID of first evt     = " << evt->getRunId() << "\n";
        std::cout << "ExtGemID of first evt = " << FT2.Digi_EvtId_start << "\n";
        std::cout << setprecision(20) << "DigiCutTstart           = " << FT2.DigiCutTstart << "\n";
        std::cout << setprecision(20) << "DigiTime of first evt   = " << DigiTime << "\n";
        ...
        T->GetEntry(Digi_i);
        DigiTime = evt->getTimeStamp();
        std::cout << setprecision(20) << "Tstop  of the DigiFile  = " << DigiTime << "\n";
        do {
            T->GetEntry(Digi_i);
            DigiTime = evt->getTimeStamp();
            Digi_i--;
            // if (Digi_i % 1000 == 0) {
            //    printf("DigiCutTstop=%20.18g    DigiTime=%20.18g \n", FT2.DigiCutTstop, DigiTime);
            // }
        } while (DigiTime > FT2.DigiCutTstop && Digi_i > 0);

        Digi_i++;
        Digi_Stop = Digi_i;
        FT2.Digi_EvtId_stop = evt->getEventId();
        LastEvtRunID = evt->getRunId();
        Tstop_Run = evt->getTimeStamp();
        std::cout << "RunID of last evt     = " << LastEvtRunID << "\n";
        std::cout << setprecision(20) << "ExtGemID of last evt = " << FT2.Digi_EvtId_stop << "\n";
        std::cout << setprecision(20) << "DigiCutTstop           = " << FT2.DigiCutTstop << "\n";
        std::cout << setprecision(20) << "DigiTime of last evt  = " << DigiTime << "\n";

        //Tsart and Tstop from DigiFile
    } else {
         printf("---------------------------------------------------\n");
        printf("Digi times from DigiFile\n");
        printf("---------------------------------------------------\n");
        T->GetEntry(Digi_i);
        DigiTime = evt->getTimeStamp();
        Tstart_Run = evt->getTimeStamp();
    ...
    std::cout << "You are using " << Digi_Stop-Digi_Start << " events" << std::endl;
    ...
    unsigned int bin_evt(0);
    bool on_gaps = false;
    for (Digi_i = Digi_Start; Digi_i <= Digi_Stop; Digi_i++) {
        ...
        if ((first_bin) || (New_FT2_Entry)) {
            ...
            if (Current_FT2_Entry % 100 == 0) {
                printf("New Entry %d\n", Current_FT2_Entry);
                cout << "Digi File at " << double(Digi_i-Digi_Start) / double(Digi_Stop-Digi_Start)*100.0 << "% " << endl;
            }
        ...
        if (Digi_EvtId != Merit_EvtId) {
            ...
            //update live time and recon dead time on screen
            if (FT2.verbose) {
                printf(" evt num for this bin=%d fraction=%e current recon deadtime=%e livetime=%e  curr livetime=%e  old livetime=%e\n",
                        bin_evt,
                        fraction,
                        FT2.DT.ReconDeadTime[Current_FT2_Entry],
                        Current_LiveTime - FT2.DT.Tstart_LiveTime[Current_FT2_Entry],
                        Current_LiveTime,
                        Old_LiveTime
                        );
            }
